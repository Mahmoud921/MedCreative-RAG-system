...
def context_is_usable(
    ...
    """
    Determines whether Chroma returned any guideline documents.

    We intentionally do NOT reject results based on the distance threshold.
    This prevents legitimate CKD questions such as:
        "What is CKD?"
        "What are the symptoms of CKD?"
        "What is eGFR?"

    from being incorrectly rejected because their vector distance is above
    the configured threshold.
    """

    if not raw_results:
        logger.warning("No documents were retrieved from Chroma.")
        return False

    logger.info(
        f"Context accepted: {len(raw_results)} guideline document(s) retrieved. "
        f"Distance threshold ({distance_threshold}) is not used for rejection."
    )
